...
for (const file of files) {
  ...
  const blurRegex = new RegExp(`${VARIANT_PREFIX}backdrop-blur(?:-\\w+)?\\s*`, 'g');
  ...
  const surfaceColors = 'card|input|popover|muted|background|sidebar|accent|secondary';
  ...
  const bgBorderRegex = new RegExp(
    `(${VARIANT_PREFIX})bg-border\\/\\d+`,
    'g'
  );
