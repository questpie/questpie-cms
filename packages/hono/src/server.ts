...
export function questpieHono(
	...
	// CMS instance middleware
	app.use("*", async (c, next) => {
		c.set("cms", cms);
		await next();
	});

	// Auth middleware - validate session and populate user
	app.use("*", async (c, next) => {
		if (!cms.auth) {
			c.set("user", null);
			await next();
	...
	// Storage upload
	// ========================================================================
	app.post(`${basePath}/storage/upload`, async (c) => {
	...
	// ========================================================================
	app.get(`${basePath}/cms/:collection`, async (c) => {
	...
	app.post(`${basePath}/cms/:collection`, async (c) => {
		const collection = c.req.param("collection");
		const context = c.get("cmsContext");
		const crud = cms.api.collections[collection as any];
	...
	app.get(`${basePath}/cms/:collection/:id`, async (c) => {
		const collection = c.req.param("collection");
	...
	app.patch(`${basePath}/cms/:collection/:id`, async (c) => {
		const collection = c.req.param("collection");
		const id = c.req.param("id");
		const context = c.get("cmsContext");
		const crud = cms.api.collections[collection as any];
	...
	app.delete(`${basePath}/cms/:collection/:id`, async (c) => {
		const collection = c.req.param("collection");
		const id = c.req.param("id");
		const context = c.get("cmsContext");
		const crud = cms.api.collections[collection as any];
	...
	// Globals routes
	// ========================================================================
	app.get(`${basePath}/cms/globals/:global`, async (c) => {
	...
	app.patch(`${basePath}/cms/globals/:global`, async (c) => {
		const global = c.req.param("global");
		const context = c.get("cmsContext");
		const body = await c.req.json();
