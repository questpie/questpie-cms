...
Hono adapter for QUESTPIE with unified client combining CMS CRUD operations and Hono RPC.
...
const app = new Hono().route("/", questpieHono(cms)).listen(3000);
...
If you prefer a pure HTTP client without Hono RPC, use the shared client from `questpie`:
