...
describe("realtime", () => {
	...
	describe("change logging", () => {
		it("logs realtime changes for create/update/delete", async () => {
			...
			const testModule = defineQCMS({ name: "realtime-test" })
				.collections({ posts })
				.locale({
			...
			setup = await buildMockCMS(testModule, { realtime: { adapter } });
		...
		it("logs bulk update/delete operations", async () => {
			...
			const testModule = defineQCMS({ name: "realtime-test" }).collections({
				posts,
			});
	...
	describe("WHERE filtering", () => {
		it("only notifies subscribers with matching WHERE filter", async () => {
			const adapter = new MockRealtimeAdapter();
			const messages = defineCollection("messages")
				.fields({
	...
	describe("WITH dependency tracking", () => {
		it("notifies subscribers when related resource changes", async () => {
			...
			const routes = createCMSAdapterRoutes(setup.cms, { accessMode: "user" });
	...
	describe("global subscriptions", () => {
		it("re-sends snapshots when global changes", async () => {
			...
			const testModule = defineQCMS({ name: "realtime-test" }).globals({
				settings,
			});
		...
		it("global subscriptions with WITH referencing collections", async () => {
			...
			const settings = defineGlobal("settings")
				.fields({
					...
					defaultCategoryId: uuidCol("default_category_id").references(
						() => categories.table.id,
					),
				})
				.relations(({ one }) => ({
	...
	describe("edge cases", () => {
		it("handles multiple subscribers with different filters", async () => {
			const adapter = new MockRealtimeAdapter();
			const orders = defineCollection("orders")
				.fields({
					status: text("status").notNull(),
		...
		it("handles empty WHERE filter (matches all)", async () => {
			const adapter = new MockRealtimeAdapter();
			const logs = defineCollection("logs")
