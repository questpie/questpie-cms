...
const comments: any = collection("comments")
	.fields({
...
const restrictedCategories = collection("restricted_categories")
	.fields({
		name: text("name").notNull(),
	})
	.relations(({ many }) => ({
...
const polymorphicComments = collection("polymorphic_comments")
	.fields({
		content: text("content").notNull(),
...
const articleTagJunction = collection("article_tag_junction")
	.fields({
...
const categoryTags = collection("category_tags")
	.fields({
		categoryId: uuid("category_id")
			.notNull()
			.references(() => categories.table.id),
...
const productTags = collection("product_tags")
	.fields({
