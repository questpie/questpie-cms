...
```typescript
import { collection } from "@questpie/core/server";
...
```typescript
const articles = collection("articles").build();
const tags = collection("tags").build();
...
const articlesWithRelation = collection("articles")
