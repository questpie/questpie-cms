...
import { History, User, Clock, ChevronDown, ChevronRight, RotateCcw } from "lucide-react";
...
export interface VersionHistoryProps<T extends QCMS<any, any, any>> {
	/**
	 * Collection name
	 */
	collection: string;
...
export function VersionHistory<T extends QCMS<any, any, any>>({
	collection,
	itemId,
	allowRestore = false,
	onRestore,
		...
		queryFn: async () => {
			...
			return [
				...
				},
				{
					...
					action: "updated",
					changes: [
						{ field: "scheduledAt", oldValue: "2025-01-15T10:00:00", newValue: "2025-01-20T14:00:00" },
					],
					data: {},
				},
				{
				...
				{versions.map((version, index) => {
					...
					return (
						<div key={version.id}>
							...
							<Collapsible open={isExpanded} onOpenChange={() => toggleVersion(version.id)}>
								<div className="flex items-start gap-3">
									...
									<div className="flex-1 space-y-1">
										...
										{/* Changes Summary */}
										{version.changes && version.changes.length > 0 && !isExpanded && (
