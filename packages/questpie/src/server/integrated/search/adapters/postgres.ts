...
export class PostgresSearchAdapter implements SearchAdapter {
	...
	async search(options: SearchOptions): Promise<SearchResponse> {
		...
		if (!hasQuery) {
			...
			rows = await this.browseRecordsWithAccess(conditions, accessFilters, accessCondition, limit, offset);
		} else if (mode === "hybrid") {
			...
			rows = await this.searchFTSWithAccess(tsQuery!, conditions, accessFilters, accessCondition, limit, offset);
		}
		...
		// Get total count
		const total = await this.getTotalWithAccess(conditions, tsQuery, accessFilters, accessCondition);
	...
	private buildAccessCondition(
		...
		for (const filter of accessFilters) {
			...
			const tableName = (table as any)[Symbol.for("drizzle:Name")] || collection;
	...
	private async searchFTSWithAccess(
		...
		const joins = this.buildAccessJoins(accessFilters);
		const allConditions = [...conditions, sql`${questpieSearchTable.ftsVector} @@ ${tsQuery}`];
		if (accessCondition) {
			allConditions.push(accessCondition);
		}
	...
	async indexBatch(params: IndexParams[]): Promise<void> {
		...
		const insertedRecords = await this.db
			...
			.returning({ id: questpieSearchTable.id, collectionName: questpieSearchTable.collectionName, recordId: questpieSearchTable.recordId, locale: questpieSearchTable.locale });
