 ...
 */
export type IsRequired<T, K extends keyof T> = K extends RequiredKeys<T> ? true : false;
/**
 * Checks if a key is optional in T.
 */
export type IsOptional<T, K extends keyof T> = K extends OptionalKeys<T> ? true : false;
/**
 ...
 */
export type IsLiteral<T> = T extends string ? string extends T ? false : true : T extends number ? number extends T ? false : true : T extends boolean ? boolean extends T ? false : true : false;
/**
 ...
 */
export type IsObject<T> = T extends object ? T extends readonly any[] ? false : T extends (...args: any[]) => any ? false : true : false;
/**
 ...
 */
export type UnwrapReturn<T extends (...args: any[]) => any> = Awaited<ReturnType<T>>;
/**
 * Counts the number of keys in a type.
 * Note: This is approximate and works for simple object types.
 */
export type KeyCount<T> = keyof T extends never ? 0 : [keyof T] extends [infer K] ? K extends any ? 1 : never : never;
/**
...
export type DeepPrettify<T> = {
    [K in keyof T]: T[K] extends object ? T[K] extends (...args: any[]) => any ? T[K] : DeepPrettify<T[K]> : T[K];
} & {};
/**
