...
const columnsBlock = adminModule.block("columns")
  .fields((f) => ({
    columns: f.number({ min: 2, max: 4 }).notNull().default(2),
    items: f.array({
      items: z.object({
        heading: z.string(),
        body: z.string(),
...
const pages = q.collection("pages")
  .fields((f) => ({
    title: f.text().notNull().localized(),
    slug: f.text().notNull(),
    content: f.blocks({
      types: [heroBlock, textBlock, imageBlock, columnsBlock],
    }).localized(),
    isPublished: f.boolean().notNull().default(false),
    // SEO fields
