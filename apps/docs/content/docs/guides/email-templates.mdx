...
QUESTPIE CMS uses [React Email](https://react.email/) for building email templates and [Nodemailer](https://nodemailer.com/) for sending them. This gives you the power of React components for email design with full type safety.
...
import { SmtpAdapter } from "@questpie/cms/server/integrated/mailer/adapters/smtp.adapter";
...
```typescript
import { createEtherealSmtpAdapter } from "@questpie/cms/server/integrated/mailer/adapters/smtp.adapter";
...
export const bookingConfirmation = defineEmailTemplate({
...
```typescript
import { ConsoleAdapter } from "@questpie/cms/server/integrated/mailer/adapters/console.adapter";
...
- [Hooks](/docs/core-concepts/hooks) - Trigger emails on data changes
