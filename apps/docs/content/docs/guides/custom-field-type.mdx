...
```ts
import { field, commonStringOperators } from "@questpie/questpie/server/fields";
...
const colorField = field<ColorFieldConfig, string>()({
  type: "color",
...
Server                          Admin
───────                         ─────
field({ type: "color" })  →  getMetadata() emits { type: "color" }
         ↓                              ↓
