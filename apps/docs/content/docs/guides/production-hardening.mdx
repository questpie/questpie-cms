...
- **Run migrations** — Ensure all migrations run before the app starts. Use `app.migrate()` in your startup script.
...
- **Run the worker** — Jobs require a running worker process: `app.queue.listen()`.
...
- **Reindex after migration** — After schema changes or bulk imports, run `app.search.reindex("collectionName")`.
...
```text
Web Server → API routes + Admin panel
Queue Worker → app.queue.listen()
```
...
- **No queue worker running** — Jobs pile up in the queue but nothing processes them. Always start `app.queue.listen()`.
- **Missing `pg_trgm` extension** — Search fails without it. Add it in your initial migration.

## Related Pages

- [Docker Deployment](/docs/guides/docker-deployment) — Containerize and deploy with Docker
