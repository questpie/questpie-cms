...
const services = q.collection("services")
  .fields((f) => ({
    ...
    duration: f.number({ min: 15 }).notNull(),  // minutes
    price: f.number({ min: 0, precision: 10, scale: 2 }).notNull(),
    isActive: f.boolean().notNull().default(true),
  }))
  .title((t) => t.name)
...
const barbers = q.collection("barbers")
  .fields((f) => ({
    name: f.text().notNull(),
    email: f.email().notNull(),
    phone: f.text(),
    avatar: f.upload(),
    bio: f.textarea(),
    services: f.relation("services", { type: "manyToMany", through: "barberServices" }),
  }))
  .title((t) => t.name)
  .options({ timestamps: true })
  .build();
...
const appointments = q.collection("appointments")
  .fields((f) => ({
    barber: f.relation("barbers").notNull(),
    service: f.relation("services").notNull(),
    customerName: f.text().notNull(),
    customerEmail: f.email().notNull(),
    customerPhone: f.text(),
    date: f.date().notNull(),
    startTime: f.time().notNull(),
    endTime: f.time().notNull(),
    status: f.select(["pending", "confirmed", "completed", "cancelled"]).notNull().default("pending"),
    notes: f.textarea(),
