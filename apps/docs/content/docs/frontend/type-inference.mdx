...
f.text({ required: true })    →    AppCollections type    →    client.collections.posts.find()
...
The client uses `AppConfig` and `AppFunctions` generics:

```ts
const client = createClient<AppConfig, AppFunctions>({
