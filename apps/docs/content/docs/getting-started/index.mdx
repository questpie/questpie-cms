...
Client (qa + admin client module)
  ...
  → widget/page registries               — client extensions
```
...
- **Duplicating schema in the client** — The server is the single source of truth. The client admin config should only handle registries and rendering overrides.
