...
The client builder handles rendering concerns — branding, locale, and custom UI. It receives all schema information from the server via introspection.
...
3. **Client config is rendering-only** — `qa()` handles branding and UI customization, not data structure
