...
description: Build a complete CMS with multiple collections, relations, sidebar, and dashboard
---

# Your First CMS

This walkthrough goes beyond the quickstart. You'll build a mini content management system with multiple collections, relations between them, a custom sidebar, reactive fields, and a typed RPC function — the same patterns used in the Barbershop reference project.
...
export const cms = q({ name: "my-first-cms" })
  .use(adminModule)
...
- **Duplicating sidebar items** — Collections not listed in the sidebar are auto-added. If you list them explicitly, remove them from the auto section or they'll appear twice.
