...
export function createMyFrameworkAdapter(cms, options = {}) {
  const handler = createRequestHandler(cms, options);
...
```typescript
// express-adapter.ts
import { createRequestHandler, type QCMS } from "questpie";
...
export function createExpressAdapter(
  ...
  const handler = createRequestHandler(cms, { basePath });
...
The `createRequestHandler` accepts these options:
...
<Cards>
  <Card href="/docs/adapters" title="Framework Adapters" description="See all available adapters" />
  <Card href="/docs/backend" title="Backend API" description="CMS API reference" />
</Cards>
