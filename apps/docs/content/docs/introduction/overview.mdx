...
- Automatic `questpie_assets` collection
...
GET    /api/collections/posts/:id       # Get post by ID
...
GET    /api/storage/:path               # Serve file

POST   /api/auth/sign-in                # Authentication endpoints
...
app.route("/api", questpieHono(cms));
...
- RESTful API at `/api/collections/posts`
