...
- **Modular composition** - Create reusable modules with `.use()` for features like auth, storage, or custom collections
- **Progressive enhancement** - Use callbacks to build on previous configurations: `.auth((prev) => ({ ...prev, plugins: [...] }))`
...
- **Start simple** - Use `defineQCMS({ name: 'app' })` for full control, or `createCoreModule()` for batteries-included setup
- **Compose modules** - Merge modules with `.use()` to add features from other packages or your own code
- **Progressive enhancement** - Build on previous configurations using callbacks

```typescript
// Create reusable modules
const orgModule = defineQCMS({ name: "org" }).auth({
  plugins: [organization()],
});

const twoFactorModule = defineQCMS({ name: "2fa" }).auth({
  plugins: [twoFactor()],
});

// Compose them together
const cms = defineQCMS({ name: "app" })
  .use(orgModule) // ✅ Merge organization auth
  .use(twoFactorModule) // ✅ Merge two-factor auth
  .collections({ posts, products })
  .build({
    /* ... */
  });

// All auth methods are available and typed:
cms.auth.api.createUser(); // From default auth
cms.auth.api.createOrganization(); // From orgModule
cms.auth.api.sendTwoFactorCode(); // From twoFactorModule
```
...
| **Framework**          | Hono/Elysia/Next/TanstackStart | Express       | Koa         | N/A          |
