...
Access Control in QUESTPIE provides granular, type-safe rules for controlling who can read, create, update, and delete records. It's built directly into the collection system and enforced automatically at the database query level.
...
QUESTPIE operates in two access modes:
...
export const posts = q
  ...
  .fields((f) => ({
    ...
    authorId: f.text({
      required: true,
      access: {
        update: ({ user }) => user?.role === "admin", // Only admins can change author
      },
    }),
  }))
  .access({
...
Set global access defaults for all collections and globals in your CMS config:

```typescript
const cms = q({ name: "app" }).build({
  defaultAccess: {
    // Require authentication for all operations by default
...
- Collections and globals **without** explicit `.access()` rules inherit from `defaultAccess`
- Collections and globals **with** explicit `.access()` rules override the defaults
- Use `defaultAccess` to set a baseline security policy (e.g., "require auth everywhere")
- Override per-collection for specific requirements

### Example: Admin-Only by Default
...
// Public collection - overrides default
const posts = q.collection("posts")
  .fields({ ... })
  .access({
    read: true, // Everyone can read
    create: ({ session }) => !!session, // Only logged-in users
  });

// Admin-only collection - uses default (no .access() needed)
const internalNotes = q.collection("internal_notes")
  .fields({ ... });
  // Inherits defaultAccess (admin-only)
```

### Field-Level Access Location

Field-level access is defined on each field definition (inside `.fields((f) => ...)`), not in `defaultAccess`:
...
### System Mode Bypasses Defaults

System access mode bypasses all access control, including `defaultAccess`:

```typescript
// System context has full access regardless of defaultAccess
const systemCtx = await cms.createContext({ accessMode: "system" });
await cms.api.collections.posts.find({}, systemCtx); // Bypasses all checks
```
...
- `create` - Who can set this field during create
- `update` - Who can modify this field during update
...
app.get("/api/posts", async (c) => {
  ...
  return app.api.collections.posts.find({}, context);
});
...
q.collection("posts")
  .fields((f) => ({
    authorEmail: f.email({
      access: {
        read: ({ user }) => user?.role === "admin", // Only admins can see email addresses
      },
    }),
  }))
  .access({
...
const anonPosts = await app.api.collections.posts.find({}, anonContext);
...
const userPosts = await app.api.collections.posts.find({}, userContext);
...
const adminPosts = await app.api.collections.posts.find({}, adminContext);
...
QUESTPIE integrates with Better Auth for authentication:
...
- [Authentication](/docs/guides/authentication) - Set up Better Auth
- [Custom Adapters](/docs/guides/custom-adapters) - Build your own integrations
