...
```ts
import { app, createContext } from "@/questpie/server/.generated";
...
// Factory functions that produce typed hooks
export const { useCollectionList, useCollectionItem } = createAdminHooks<AppConfig>();
```
...
- Add, rename, or delete collection/global/function/job files
