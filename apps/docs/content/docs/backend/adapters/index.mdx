...
const cms = q({ name: "my-cms" })
  ...
  .build({
    ...
    // KV adapter
    kv: {
      adapter: new RedisKVAdapter(redisClient),
      defaultTtl: 3600,
    },
  });
```
...
<Cards>
  <Card href="/docs/backend/adapters/storage" title="Storage" description="File uploads and serving" />
  <Card href="/docs/backend/adapters/queue" title="Queue" description="Background jobs" />
  <Card href="/docs/backend/adapters/email" title="Email" description="Email sending" />
