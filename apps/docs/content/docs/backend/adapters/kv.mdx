...
```typescript
import { RedisKVAdapter } from "questpie/kv";
...
const cms = q({ name: "my-cms" })
  .build({
    kv: {
      adapter: new RedisKVAdapter(redis),
      defaultTtl: 3600,
...
```typescript
import { UpstashKVAdapter } from "questpie/kv";
...
const cms = q({ name: "my-cms" })
  .build({
    kv: {
      adapter: new UpstashKVAdapter(redis),
    },
  });
```
...
<Cards>
  <Card href="/docs/backend/adapters/logger" title="Logger" description="Logging configuration" />
  <Card href="/docs/backend/adapters/storage" title="Storage" description="File storage" />
</Cards>
