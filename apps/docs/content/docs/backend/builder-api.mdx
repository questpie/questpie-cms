...
```typescript
const cms = q({ name: "my-cms" }).collections({
...
```typescript
const cms = q({ name: "my-cms" }).globals({
  settings: q.global("settings").fields({
    siteName: text("siteName"),
    description: text("description"),
    logo: text("logo"),
  }),

  navigation: q.global("navigation").fields({
    items: jsonb("items"),
  }),
});
```
...
### Standalone RPC Contracts

Root RPC procedures are now defined outside the CMS builder with `rpc().router(...)`.

```typescript
import { q, rpc } from "questpie";
import { z } from "zod";

const sendNewsletter = q.fn({
  schema: z.object({
    subject: z.string(),
    content: z.string(),
...
const generateReport = q.fn({
  schema: z.object({ startDate: z.string(), endDate: z.string() }),
  handler: async ({ input }) => {
    return { url: `/reports/${input.startDate}-${input.endDate}.pdf` };
  },
});

export const appRpc = rpc().router({
  sendNewsletter,
  generateReport,
});
```

Pass `appRpc` to your adapter/fetch handler:
...
```typescript
const cms = q({ name: "my-cms" }).locale({
  locales: [
...
const cms = q({ name: "my-cms" }).emailTemplates({
...
```typescript
const cms = q({ name: "my-cms" }).messages({
  en: {
...
const cms = q({ name: "my-cms" }).migrations([
  {
...
- [Collections](/docs/backend/collections) - Define data models
- [CRUD Operations](/docs/backend/crud-operations) - Query and mutate data
- [Validation](/docs/backend/validation) - Zod schema validation
- [Modules](/docs/backend/modules) - Compose features
