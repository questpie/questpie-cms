...
Call typed RPC procedures from `cmsClient.rpc.*`.
...
export const appRpc = rpc().router({
  newsletter: rpc().router({
    send: sendNewsletter,
  }),
});

type AppRpc = typeof appRpc;
...
const result = await cmsClient.rpc.newsletter.send({
...
```tsx
// components/PublishPost.tsx
import { useMutation } from "@tanstack/react-query";
import { cmsQueries } from "@/lib/cms-queries";

function PublishPost({ id }: { id: string }) {
  const mutation = useMutation(cmsQueries.rpc.posts.publish.mutation());
...
- [CRUD Operations](/docs/backend/crud-operations) - Server-side operations
- [Validation](/docs/backend/validation) - Input validation
- [Authentication](/docs/guides/authentication) - Auth integration
- [Admin UI](/docs/admin) - Admin interface
