...
Globals support many-to-many relations just like collections. This is perfect for scenarios where your global needs to reference multiple records from another collection.

**Example: Featured Services**

Define a global that highlights multiple services on your homepage:

```typescript
const services = collection("services").fields({
  name: varchar("name", { length: 255 }).notNull(),
  description: text("description"),
  price: integer("price"),
});

// Junction table linking homepage to services
const homepageServices = collection("homepage_services").fields({
  homepageId: varchar("homepage_id", { length: 36 }).notNull(),
...
// Global with many-to-many relation
export const homepage = global("homepage")
  .fields({
    heroTitle: varchar("hero_title", { length: 255 }),
    heroDescription: text("hero_description"),
  })
  .relations(({ manyToMany }) => ({
    featuredServices: manyToMany("services", {
      through: "homepage_services",
      sourceField: "homepageId",
...
await cms.api.globals.homepage.update(
  {
    featuredServices: {
      create: [
        ...
        { name: "Support", description: "24/7 help", price: 50 },
      ],
    },
  },
  ctx,
...
await cms.api.globals.homepage.update(
  {
    featuredServices: { set: [{ id: newService.id }] },
  },
  ctx,
);
```

Plain array of IDs (admin forms):

```typescript
await cms.api.globals.homepage.update(
  {
    featuredServices: [serviceId1, serviceId2, serviceId3],
  },
  ctx,
);
```
...
const homepage = await cms.api.globals.homepage.get(
  {
    with: { featuredServices: true },
  },
  ctx,
);
// Result: { heroTitle: "...", featuredServices: [{ name: "...", price: ... }] }
```
...
- [Collections](/docs/core-concepts/collections) - Multi-record data
- [Hooks](/docs/backend/hooks) - Add business logic
- [Access Control](/docs/backend/access-control) - Security
