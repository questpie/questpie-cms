...
Place route files in `functions/`. The filename becomes the route key: `get-active-barbers.ts` → `getActiveBarbers`.
...
Routes are accessible via `POST /routes/routeName`:

```bash
curl -X POST http://localhost:3000/api/routes/createBooking \
...
- [Routes API](/docs/reference/functions-api) — Route builder API reference
