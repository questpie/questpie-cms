---
title: Services
description: Singleton and request-scoped services — define once, inject everywhere via AppContext.
---

# Services

Services are reusable units of logic that get injected into `AppContext`. Define a service in `services/`, and it becomes available in every hook, function, job, and route handler.

## Defining a Service

```ts title="services/blog.ts"
import { service } from "questpie";

export class BlogService {
  computeReadingTime(content: string): number {
    const text = content.replace(/<[^>]*>/g, " ");
    const words = text.trim().split(/\s+/).filter((w) => w.length > 0).length;
    return Math.max(1, Math.ceil(words / 200));
  }

  generateSlug(title: string): string {
    return title
      .toLowerCase()
      .replace(/[^a-z0-9\s-]/g, "")
      .trim()
      .replace(/\s+/g, "-");
  }
}

export default service({
  lifecycle: "singleton",
  create: () => new BlogService(),
});
```

The filename becomes the context key: `services/blog.ts` → `{ blog }` in AppContext.

## Using Services

Services are available in every handler via context destructuring:

```ts title="collections/posts.ts"
export const posts = collection("posts")
  .fields(({ f }) => ({
    title: f.text({ label: "Title", required: true }),
    content: f.richText({ label: "Content" }),
    readingTime: f.number({ label: "Reading Time" }),
    slug: f.text({ label: "Slug" }),
  }))
  .hooks({
    beforeChange: async ({ data, blog }) => {
      if (data.content) {
        data.readingTime = blog.computeReadingTime(data.content);
      }
      if (data.title) {
        data.slug = blog.generateSlug(data.title);
      }
    },
  });
```

```ts title="functions/get-post-stats.ts"
export default fn({
  schema: z.object({ postId: z.string() }),
  handler: async ({ input, collections, blog }) => {
    const post = await collections.posts.findOne({ where: { id: input.postId } });
    return {
      readingTime: blog.computeReadingTime(post.content),
      excerpt: blog.extractExcerpt(post.content),
    };
  },
});
```

## Lifecycle

| Lifecycle | Created | Destroyed | Use for |
|---|---|---|---|
| `"singleton"` (default) | Once at app startup | App shutdown | External clients, SDKs, connection pools |
| `"request"` | Per request | End of request | Tenant-scoped DB, user-specific config |

### Singleton (default)

```ts title="services/stripe.ts"
import { service } from "questpie";
import Stripe from "stripe";

export default service({
  lifecycle: "singleton",
  create: () => new Stripe(process.env.STRIPE_SECRET_KEY!),
});
```

### Request-scoped

```ts title="services/tenant-db.ts"
import { service } from "questpie";

export default service({
  lifecycle: "request",
  deps: ["db", "session"] as const,
  create: ({ db, session }) => {
    return createScopedDb(db, session?.user?.tenantId);
  },
  dispose: (scopedDb) => scopedDb.release(),
});
```

## Dependencies

Services can depend on other services and infrastructure via `deps`. Use `as const` for type safety:

```ts title="services/analytics.ts"
import { service } from "questpie";

export default service({
  deps: ["db", "logger"] as const,
  create: ({ db, logger }) => {
    logger.info("Analytics service initialized");
    return new AnalyticsService(db);
  },
});
```

Available dependencies include: `db`, `logger`, `kv`, `email`, `queue`, `storage`, `search`, `realtime`, `session`, and any other user-defined services.

## Disposal

Optional `dispose` function for cleanup:

```ts
export default service({
  lifecycle: "singleton",
  create: () => {
    const pool = createConnectionPool();
    return pool;
  },
  dispose: async (pool) => {
    await pool.close();
  },
});
```

- **Singleton**: `dispose` called at app shutdown
- **Request**: `dispose` called at end of each request

## API Reference

```ts
service({
  lifecycle?: "singleton" | "request",  // default: "singleton"
  deps?: readonly string[],             // dependencies from AppContext
  create: (deps) => TInstance,          // factory function
  dispose?: (instance) => void | Promise<void>,  // cleanup
})
```

## Related Pages

- [Functions](/docs/backend/business-logic/functions) — Use services in function handlers
- [Hooks](/docs/backend/rules/hooks) — Use services in lifecycle hooks
- [Codegen](/docs/backend/architecture/codegen) — How services appear in AppContext
