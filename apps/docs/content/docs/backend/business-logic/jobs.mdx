...
Publish jobs from hooks, functions, or other jobs via the `queue` context:
...
Job handlers receive the same `AppContext` as functions:
...
- [Functions](/docs/backend/business-logic/functions) — Server-side logic
