...
export const siteConfig = defineGlobal("site_config").fields({
...
```typescript
export const homepage = defineGlobal("homepage").fields({
...
```typescript
export const siteConfig = defineGlobal("site_config").options({
  versioning: true,
});
...
export const blogPosts = defineGlobal("blog_posts").fields({
  /* multiple posts */
});
...
```typescript
export const siteConfig = defineGlobal("site_config").hooks({
  afterChange: async ({ data }) => {
    const cms = getCMSFromContext();
    // Invalidate caches
    await cms.queue["invalidate-cache"].publish({
...
```typescript
export const siteConfig = defineGlobal("site_config").access({
