...
Use `defineGlobal` with a fluent builder API:

```typescript
import { defineGlobal } from "@questpie/cms/server";
import { varchar, text, boolean, jsonb } from "drizzle-orm/pg-core";

export const siteConfig = defineGlobal("site_config")
  .fields({
...
```typescript
export const homepage = defineGlobal("homepage")
  .fields({
  ...
  .hooks({
    afterUpdate: async ({ data, original }) => {
      ...
      if (data.maintenanceMode !== original.maintenanceMode) {
        await cms.queue["maintenance-mode-changed"].publish({
          enabled: data.maintenanceMode,
        });

        cms.logger.warn("Maintenance mode toggled", {
          enabled: data.maintenanceMode,
        });
      }
    },
...
// Get the global
const config = await cms.api.globals.siteConfig.get(context);
...
```typescript
const updated = await cms.api.globals.siteConfig.update({
  data: {
...
```typescript
const homepage = await cms.api.globals.homepage.get({
  with: {
    featuredPost: true,
...
// Get version history
const versions = await cms.api.globals.siteConfig.findVersions({
  limit: 10,
}, context);

// Revert to a previous version
await cms.api.globals.siteConfig.revertToVersion({
...
export const siteSettings = defineGlobal("site_settings")
  ...
  .relations(({ table, one }) => ({
    logoAsset: one("questpie_assets", {
      fields: [table.logo],
      references: ["id"],
    }),
    faviconAsset: one("questpie_assets", {
...
export const homepage = defineGlobal("homepage")
  ...
  .localized(["heroTitle", "heroSubtitle", "heroCtaText", "featuredSectionTitle"])
  .relations(({ table, one }) => ({
    heroBackground: one("questpie_assets", {
  ...
  .hooks({
    afterUpdate: async ({ data }) => {
      ...
      await cms.queue["invalidate-cache"].publish({
        keys: ["homepage"],
      });
    },
  });
...
```typescript
export const seoDefaults = defineGlobal("seo_defaults")
  .fields({
...
```typescript
export const featureFlags = defineGlobal("feature_flags")
  .fields({
  ...
  .hooks({
    afterUpdate: async ({ data, original }) => {
      ...
      for (const change of changes) {
        cms.logger.info("Feature flag changed", {
          flag: change,
          oldValue: original[change],
          newValue: data[change],
        });
      }
    },
  })
...
export const navigation = defineGlobal("navigation")
  .fields({
...
export const blogPosts = defineGlobal("blog_posts")
  .fields({ /* multiple posts */ });
...
export const siteConfig = defineGlobal("site_config")
  .fields({
    ...
    enableComments: boolean("enable_comments")
...
const config = await cms.api.globals.siteConfig.get();
...
```
GET    /api/globals/site-config       # Get global
PATCH  /api/globals/site-config       # Update global
GET    /api/globals/site-config/versions  # Get version history
POST   /api/globals/site-config/revert    # Revert to version
```
