...
defineCollection("products").fields({
...
```typescript
import { softDeleteUniqueIndex } from "@questpie/cms/server";
...
```typescript
defineCollection("posts").options({ versioning: true });
...
```typescript
defineCollection("posts").options({ softDelete: true });
...
await cms.api.collections.posts.restoreById({ id: postId }, context);
```
...
```typescript
defineCollection("blogPosts").options({
