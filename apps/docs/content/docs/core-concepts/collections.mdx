...
Admin UI is configured on the **server** alongside your collection definition using `.admin()`, `.list()`, and `.form()`:
...
Field-level access is configured on field definitions:
