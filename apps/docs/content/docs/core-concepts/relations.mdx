...
Relations allow you to connect collections together, creating powerful data models that reflect real-world relationships. QUESTPIE CMS provides type-safe relations with eager loading support.
...
QUESTPIE CMS supports four types of relations:
...
export const postTags = defineCollection("post_tags")
  .fields({
    postId: uuid("post_id")
      .notNull()
...
export const tags = defineCollection("tags")
  .fields({
...
export const orderItems = defineCollection("order_items")
  .fields({
...
```typescript
export const postImages = defineCollection("post_images")
  .fields({
...
A record can belong to multiple different collection types.

### Basic Example

Comments that can be on Posts OR Products:

```typescript
export const comments = defineCollection("comments")
  .fields({
    content: text("content"),
    commentableType: varchar("commentable_type", { length: 50 }), // "posts" or "products"
    commentableId: uuid("commentable_id"), // ID of post or product
  })
  .relations(({ polymorphic }) => ({
    commentable: polymorphic({
      typeField: table.commentableType, // Which collection type
      idField: table.commentableId,     // Which record ID
      collections: {
        posts: "posts",       // Map type value to collection name
        products: "products",
      },
    }),
  }));
...
export const posts = defineCollection("posts")
  .fields({ /* ... */ })
  .relations(({ many }) => ({
    comments: many("comments"), // Inverse relation
  }));
...
### Polymorphic with Metadata

Attachments that can belong to multiple types:

```typescript
export const attachments = defineCollection("attachments")
  .fields({
  ...
  .relations(({ polymorphic }) => ({
    attachable: polymorphic({
      typeField: table.attachableType,
      idField: table.attachableId,
      collections: {
        posts: "posts",
        comments: "comments",
        users: "questpie_users",
      },
    }),
  }));
```
...
```typescript
const users = await cms.api.collections.users.find({
  with: {
    posts: {
...
const posts = await cms.api.collections.posts.find({
  with: {
    ...
    postImages: { // Direct access to junction
      with: {
        asset: true,
      },
      orderBy: { order: "asc" },
...
```typescript
const comments = await cms.api.collections.comments.find({
  with: {
...
const categories = await cms.api.collections.categories.find({
...
export const profiles = defineCollection("profiles")
  .fields({
...
- See [Creating a Collection](/docs/guides/creating-a-collection) for step-by-step examples
...
- **Examples**: `examples/hono-barbershop/src/cms.ts:79-155`
