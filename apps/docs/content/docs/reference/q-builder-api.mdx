...
**Parameter:** `Record<string, CollectionBuilder>` — map of collection names to collection builders.
...
**Parameter:** `Record<string, GlobalBuilder>` — map of global names to global builders.
...
After build, export `AppCMS` once and reuse it in hooks/access/RPC/jobs:
