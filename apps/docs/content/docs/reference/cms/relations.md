...
```typescript
import { collection, fields } from "@questpie/cms/server";
...
```typescript
// Junction table
const postsTags = defineCollection("posts_tags")
	.fields({
...
const postsTags = defineCollection("posts_tags");  // Good
const tagsProducts = defineCollection("tags_products");  // Good
const tag_post = defineCollection("tag_post");     // Avoid
```
