...
.hooks({
  ...
  beforeTransition?: TransitionHookFn | TransitionHookFn[];
  afterTransition?: TransitionHookFn | TransitionHookFn[];
})
```
...
| `app` | `TApp` | app instance — use `getApp<App>(app)` or `getContext<App>(ctx)` |
| `session` | `Session \| null` | Authenticated user session |
| `locale` | `string` | Current locale |
| `accessMode` | `string` | `"system"` or `"user"` |
| `db` | `Database` | Database client — use `getDb<App>(db)` or `getContext<App>(ctx)` |
...
```ts
(ctx: { data: TSelect; fromStage: string; toStage: string; app: TApp; session?: Session; db: Database }) => void | Promise<void>
```

Runs before a workflow stage transition. Throw to abort the transition. Only fires for `transitionStage()` calls (requires `workflow` enabled).
...
Runs after a workflow stage transition completes. Use for notifications, logging, or side effects.
