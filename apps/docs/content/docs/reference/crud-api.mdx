...
Every collection exposes CRUD operations via `app.api.collections.{name}`. These methods are type-safe — field names, input types, and return types are inferred from your collection definition.
...
| `stage` | `string` | Workflow stage to read from (requires `workflow`) |
...
```ts
const total = await app.api.collections.posts.count({
  where: { status: "published" },
});
```
...
```ts
const updated = await app.api.collections.posts.update({
  where: { id: "post-123" },
...
```ts
const updated = await app.api.collections.posts.updateMany({
...
```ts
await app.api.collections.posts.deleteMany({
...
## transitionStage(params, context?)

Transition a record to a different workflow stage (requires `workflow` enabled). No data is mutated — only a version snapshot at the target stage is created.

```ts
const result = await app.api.collections.posts.transitionStage(
...
|---|---|---|
| `id` | `string` | Record ID to transition |
| `stage` | `string` | Target workflow stage name |
| `scheduledAt` | `Date` (optional) | Schedule the transition for a future time (requires queue adapter) |

### Behavior

- Validates the target stage exists in the workflow config
- Enforces transition guards (e.g. `draft` can only go to `review`)
- Checks `access.transition` permission (falls back to `access.update`)
- Fires `beforeTransition` and `afterTransition` hooks
- Creates a version snapshot with the new stage

### Response

Returns the transitioned `TSelect`.
...
```ts
{
  ...
  stage?: string;                   // Workflow stage for stage-aware reads/writes
