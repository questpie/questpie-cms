...
These add TypeScript types without any runtime effect. Use them to get type-safe access to context values:
...
.hooks({
  afterChange: async ({ app, db, session }) => {
    ...
    const typedDatabase = typedDb<AppCMS>(db);
    const typedSess = typedSession<AppCMS>(session);

    await cms.kv.set("cache:key", "value");
    await typedDatabase.execute(sql`SELECT 1`);
  },
})
...
These actually retrieve context from AsyncLocalStorage at runtime:

```ts
import { getContext, tryGetContext, runWithContext } from "questpie";

// Get context (throws if not in scope)
const { db, session, locale } = getContext<AppCMS>();

// Safe access (returns undefined if not in scope)
const ctx = tryGetContext();
if (ctx?.locale) {
  // Use locale
}

// Run code within a context scope
await runWithContext({ app: cms, session, db, locale: "sk" }, async () => {
  // Inside here, getContext() works
});
```

### Automatic Context Propagation

Context automatically propagates through nested API calls via AsyncLocalStorage. This means locale and accessMode are inherited without explicit passing:

```ts
// Outer call sets locale
await cms.api.collections.pages.find({}, { locale: "sk" });

// Inside hooks or nested calls, locale is inherited automatically
.hooks({
  afterRead: async ({ app }) => {
    const cms = typedApp<AppCMS>(app);
    
    // This automatically uses locale: "sk" from parent context
    const related = await cms.api.collections.posts.find({
      where: { featured: true },
    });
    // No need to pass { locale: "sk" } - it's inherited!
  },
})
```
...
```ts
import { withTransaction, typedApp } from "questpie";
...
```ts
import { onAfterCommit, typedApp } from "questpie";
...
```ts
import { withTransaction, onAfterCommit, typedApp } from "questpie";
...
- [Type-Safe Contexts](/docs/server/type-safe-contexts) — Full type helper reference
