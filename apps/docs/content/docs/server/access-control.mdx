...
- **`"system"` mode** — Internal backend calls (hooks, jobs, RPC handlers). Access rules are bypassed. This is the default when calling `app.api.collections.*` directly.
...
For collections with `workflow` enabled, add `access.transition` to control who can transition between stages. If not defined, it falls back to `access.update`:

```ts
.access({
  update: () => true,              // anyone can edit
  transition: ({ session }) => {   // only admins can transition stages
    return (session?.user as any)?.role === "admin";
  },
})
```

This is useful when editors should be able to modify content but only certain roles can publish or advance the workflow.

## Global Access

Globals support `read`, `update`, and `transition` operations (no create/delete since globals are singletons):
...
Define field-specific rules in the collection's `.access()` method:

```ts
export const employees = qb
  .collection("employees")
  .fields((f) => ({
    name: f.text({ required: true }),
    email: f.text({ required: true }),
    ssn: f.text({ maxLength: 20 }),
    salary: f.number({}),
  ...
  .access({
    ...
    // Field-level access — source of truth for per-field authorization
    fields: {
      ssn: {
...
.access({
  delete: async ({ session, data, app, db }) => {
    const app = getApp<App>(app);
    const typedDb = getDb<App>(db);
...
For full context typing in one line:
...
Set app-wide defaults that apply to collections without explicit `.access()`:

```ts
const app = questpie(builder).build({
  // ...
  defaultAccess: {
...
const allRecords = await app.api.collections.posts.find({});

// User mode — enforces access rules
const userRecords = await app.api.collections.posts.find(
  {},
