...
Icons use `c.icon("ph:icon-name")` on the server, rendered via `@iconify/react`:
...
### Collection with Full Configuration (Server-Side)
...
```typescript
// Wrong — use c.icon() helper on the server
...
- [No .build() Method](/docs/admin/patterns/no-build-method) - Why admin doesn't use .build()
- [Icons as Components](/docs/admin/patterns/icons-as-components) - Proper icon usage
