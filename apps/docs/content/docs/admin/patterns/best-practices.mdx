...
Keep sidebar configuration in your main CMS file on the server:
    ...
    sections: [
      s.section({
        ...
        items: [
          ...
          { type: "collection", collection: "users", icon: c.icon("ph:users") },
        ],
      }),
      s.section({
        id: "settings",
        ...
        icon: c.icon("ph:gear"),
        items: [
          { type: "global", global: "settings", icon: c.icon("ph:gear") },
        ],
      }),
    ],
  }))
...
- [ ] Enable search and filters
- [ ] Use `c.icon("ph:icon-name")` for icons
...
- [No .build() Method](/docs/admin/patterns/no-build-method) - Understanding admin config
- [Authentication](/docs/guides/authentication) - Set up user authentication
