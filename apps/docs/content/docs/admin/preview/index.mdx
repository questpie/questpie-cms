...
export const pages = qb
  .collection("pages")
  .admin(({ c }) => ({ label: "Pages", icon: c.icon("ph:file-text") }))
  .preview({
    url: (values) => {
...
- [Setup Guide](/docs/admin/preview/setup) - Complete setup instructions
- [Click-to-Focus](/docs/admin/preview/click-to-focus) - Navigate from preview to fields
