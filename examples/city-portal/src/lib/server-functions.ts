...
export const getCities = createServerFn({ method: "GET" }).handler(async () => {
	...
	const result = await cms.api.collections.cities.find(
		{
			where: { isActive: true },
			orderBy: { name: "asc" },
		},
...
export const getSiteSettings = createServerFn({ method: "GET" })
	...
	.handler(async ({ data }) => {
		const ctx = await createServerContext();
		const cityResult = await cms.api.collections.cities.find(
			{
...
export const getHomepage = createServerFn({ method: "GET" })
	...
	.handler(async ({ data }) => {
		...
		// Try "home" then "index"
		let page = await cms.api.collections.pages.findOne(
			{ where: { slug: "home", city: city.id, isPublished: true } },
			ctx,
		);
		if (!page) {
			page = await cms.api.collections.pages.findOne(
...
export const getNewsList = createServerFn({ method: "GET" })
	...
	.handler(async ({ data }) => {
		...
		const result = await cms.api.collections.news.find(
			{
				where,
				limit: data.limit || 20,
...
export const getNewsBySlug = createServerFn({ method: "GET" })
	...
	.handler(async ({ data }) => {
		...
		const article = await cms.api.collections.news.findOne(
			{
				where: {
					slug: data.newsSlug,
					city: city.id,
...
export const getAnnouncementsList = createServerFn({ method: "GET" })
	...
	.handler(async ({ data }) => {
		...
		const result = await cms.api.collections.announcements.find(
			{
				where,
...
export const getDocumentsList = createServerFn({ method: "GET" })
	...
	.handler(async ({ data }) => {
		...
		const result = await cms.api.collections.documents.find(
			{
				where,
				limit: data.limit || 50,
...
export const getContactPageData = createServerFn({ method: "GET" })
	...
	.handler(async ({ data }) => {
		...
		const result = await cms.api.collections.contacts.find(
			{
				where: { city: city.id },
				orderBy: { order: "asc" },
				limit: 100,
...
export const submitContactForm = createServerFn({ method: "POST" })
	...
	.handler(async ({ data }) => {
		...
		await cms.api.collections.submissions.create(
			{
				city: city.id,
				name: data.name,
				email: data.email,
