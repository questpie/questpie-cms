...
export const barbersFeaturedBlock = builder
	...
	.prefetch(async ({ values, locale }) => {
		const barberIds = (values.barberIds as string[]) || [];
		if (barberIds.length === 0) return { barbers: [] };

		const barbersResult = await client.collections.barbers.find({
			where: {
				id: { in: barberIds },
				isActive: true,
			},
		...
		// Maintain order from barberIds
		const barbersMap = new Map(barbersResult.docs.map((b: any) => [b.id, b]));
		const orderedBarbers = barberIds
			.map((id: string) => barbersMap.get(id))
			.filter(Boolean);

		return {
			barbers: orderedBarbers.map((b: any) => ({
				id: b.id,
				name: b.name,
				slug: b.slug,
				bio: b.bio,
